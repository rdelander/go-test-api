...
type mockUserRepository struct {
	upsertFunc      func(ctx context.Context, req *user.CreateUserRequest) (*user.UserResponse, error)
	listFunc        func(ctx context.Context) ([]*user.UserResponse, error)
	listByEmailFunc func(ctx context.Context, email string) ([]*user.UserResponse, error)
}

func (m *mockUserRepository) Upsert(ctx context.Context, req *user.CreateUserRequest) (*user.UserResponse, error) {
	if m.upsertFunc != nil {
...
func (m *mockUserRepository) List(ctx context.Context) ([]*user.UserResponse, error) {
	if m.listFunc != nil {
...
func (m *mockUserRepository) ListByEmail(ctx context.Context, email string) ([]*user.UserResponse, error) {
...
func TestUserHandler_Create(t *testing.T) {
	tests := []struct {
		name           string
		body           string
		mockUpsert     func(ctx context.Context, req *user.CreateUserRequest) (*user.UserResponse, error)
		expectedStatus int
...
func TestUserHandler_List(t *testing.T) {
	tests := []struct {
		name            string
		query           string
		mockList        func(ctx context.Context) ([]*user.UserResponse, error)
		mockListByEmail func(ctx context.Context, email string) ([]*user.UserResponse, error)
		expectedStatus  int
		expectedCount   int
		expectError     bool
	}{
		...
		},
		{
			...
			mockListByEmail: func(ctx context.Context, email string) ([]*user.UserResponse, error) {
				...
				return []*user.UserResponse{{ID: "1", Name: "John Doe", Email: "john@example.com"}}, nil
			},
			expectedStatus: http.StatusOK,
			expectedCount:  1,
			expectError:    false,
